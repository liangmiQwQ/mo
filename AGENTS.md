...
`ghm setup` initializes config by checking `git`/`gh` availability, collecting root directory/shells, optionally collecting aliases for `ghm clone`/`ghm list`, explicitly asking whether to alias `ghm cd`, then writing config and syncing shellrc.
