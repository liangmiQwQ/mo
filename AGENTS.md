...
Local development wrappers are managed by `vp run dev:mo`, `vp run dev:moi`, `vp run dev:mouni`, and `vp run dev:moiuni`, targeting the matching `~/.local/bin/mo*` or `~/.local/bin/moi*` commands; each wrapper runs Vite+ pack and regenerates `dist-moi` quietly before executing the bundled `bin` entry with `node`.
