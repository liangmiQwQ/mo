...
`vp run build` runs Vite+ pack and generates the ignored `dist-moi` alias package for `@liangmi/moi`; the alias package exposes `moi`, `moi-get-root`, and `moi-inner` with the same version as `@liangmi/mo`.
Local development wrappers are managed by `vp run dev:i` and `vp run dev:uni`, targeting `~/.local/bin/mo`, `~/.local/bin/mo-get-root`, `~/.local/bin/mo-inner`, `~/.local/bin/moi`, `~/.local/bin/moi-get-root`, and `~/.local/bin/moi-inner`; each wrapper runs Vite+ pack and regenerates `dist-moi` quietly before executing the bundled `bin` entry with `node`.
